# Playing with ASCII vector graphics...
...
class MakeGrid:
    def __init__(self, size):
        ...
        for x in range(size):
            for y in range(size):
                self.grid[y,x] = bgsym # User-defined background.
...
for i in range(linelen):
    for l in range(3):
        ...
        my_grid.grid[gridsize - i, gridsize - i/(l+2)] = pointsym
        my_grid.grid[i, i/(l+1)] = pointsym
        my_grid.grid[gridsize - i, i/(l+3)] = pointsym

# Just for fun, random points
